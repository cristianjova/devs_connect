...
export const addEducation = (formData, history) => async dispatch => {
  try {
    ...
    dispatch(setAlert('Estudios Agregados', 'success'));
...
export const deleteAccount = () => async dispatch => {
  if (window.confirm('¿Estas seguro? Esta acción no se puede deshacer!')) {
    try {
      ...
      dispatch(setAlert('Tu cuenta fue eliminada permanentemente'));
    } catch (err) {
      dispatch({
        type: PROFILE_ERROR,
        payload: { msg: err.response.statusText, status: err.response.status }
